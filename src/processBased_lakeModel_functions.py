...
def boundary_module(
    ...
    if ice:
        ...
        piston_velocity = k600_to_kgas(k600 = k600, temperature = Tair, gas = "N2O")/86400
        IP_m = IP
...
def transport_module(
    ...
    # DERIVED TEMPERATURE OUTPUT FOR NEXT MODULE
        ...
        for k in range(1,j-1):
            mn[k] = alpha[k]/2 * o2n[k-1] + (1-  alpha[k]) * o2n[k] + alpha[k]/2 * o2n[k+1]
        ...
        for k in range(1,j-1):
            mn[k] = alpha[k]/2 * docrn[k-1] + (1- alpha[k]) * docrn[k] + alpha[k]/2 * docrn[k+1]
        ...
        for k in range(1,j-1):
            mn[k] = alpha[k]/2 * docln[k-1] + (1- alpha[k]) * docln[k] + alpha[k]/2 * docln[k+1]
        ...
        for k in range(1,j-1):
            mn[k] = alpha[k]/2 * algn[k-1] + (1 - alpha[k]) * algn[k] + alpha[k]/2 * algn[k+1]
        ...
        for k in range(1,j-1):
            mn[k] = alpha[k]/2 * nutrn[k-1] + (1 - alpha[k]) * nutrn[k] + alpha[k]/2 * nutrn[k+1]
...
def run_wq_model(
  ...
  for idn, n in enumerate(times):
    ...
    internal_energy_1 = 1/area[0] * sum(un * calc_dens(un) * area) *dx * 4186
    ...
    docr_respirationm[:, idn] = 1
    docl_respirationm[:, idn] = 1
    pocr_respirationm[:, idn] = 1
    pocl_respirationm[:, idn] = 1
    nppm[:, idn] = 1
    algae_growthm[:, idn] = 1
    algae_grazingm[:, idn] = 1
    ...
    internal_energy_2 = 1/area[0] * sum(u * calc_dens(u) * area) *dx * 4186
    print(internal_energy_2 + external_energy - internal_energy_1)
